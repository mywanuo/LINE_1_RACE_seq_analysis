...
parser = argparse.ArgumentParser(description='Analyze repatmasker output file - identify soft clipped fragments')

parser.add_argument('--input', dest='inputfile', action='store', help='Input file from Parsing-RepeatMasker-Outputs/parseRM.pl (required)',required=True)
parser.add_argument('--fasta', dest='fastafile', action='store', help='Fasta file with sequences provided as an input for repeatmasker (required)',required=True)
...
for row in data.itertuples():
	...
	if (seq_name in seq_names):
		...
		seq_descr="\tclip5: " + str(clip5_seq) + "\tclip3: "+clip_seq+"\tpos: "+str(LINE_end_in_ref) + "\tref: " + str(LINE_name)
