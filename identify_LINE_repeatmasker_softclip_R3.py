...
parser.add_argument('--input', dest='inputfile', action='store', help='Input file(required)',required=True)
parser.add_argument('--fasta', dest='fastafile', action='store', help='Fasta file(required)',required=True)
parser.add_argument('--output', dest='output', action='store', help='Output tsv file (required)',required=True)
parser.add_argument('--glob', dest='glob', action='store', help='Custom glob for files to analyze',required=False)
...
for row in data.itertuples():
	...
	if (seq_name in seq_names):
		...
		LINE_seq = str(LINE_seq_obj.reverse_complement()) #reverse complement the LINE sequence
		clip_seq=whole_seq[:LINE_start_in_seq-1]
		clip_seq_obj = Seq(str(clip_seq))
		clip_seq = str(clip_seq_obj.reverse_complement()) #reverse complement clipped fragment

		#produce output sequence:
		seq_descr="\tclip5: \tclip3: "+clip_seq+"\tpos: "+str(LINE_end_in_ref) + "\tref: " + str(LINE_name)
		processed_seq = SeqRecord(Seq(str(LINE_seq)),id=whole_seq_id,description=str(seq_descr))
