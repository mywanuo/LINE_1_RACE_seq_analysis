...
def analyze_tails(R1,R2,transcript,sample_name,localization,replicate,condition,cell_line,primer_name,person):
	...
	for record in SeqIO.parse(R1, "fasta"):
		...
		if(match_CTGAC_R5):
			...
			tails_results[seq_id]['CTGAC_R5']=1 #mark the presence of CTGAC in clipped fragment (prerequisite of analysis)
			clip3_R5_length=len(clipped_R5)
			#check if clipped fragment contains other stuff than A/U/AU tails (possible heterogenity of 3'end of LINE1)
			match_heterogenous_end_tail = re.search(regex_for_heterogenous_end_tail,clipped_R5)
			if(match_heterogenous_end_tail):
				...
				heterogenous_end = match_heterogenous_end_tail.group("genomic_fragment")
		...
		if(clip3_R3_length>0):
			...
			if(match_heterogenous_end_tail_R3):
				heterogenous_end = match_heterogenous_end_tail_R3.group("genomic_fragment")
				number_heterogenous_nucleotides=len(heterogenous_end)
		...
		if (R5_mapping_pos!=-1):
			...
			if ((R5_last_mapped_nucleotide=='A') & ((R5_first_clipped_nucleotide=='A') | (R5_first_clipped_nucleotide=='NA'))):
				...
				if(match_read_nucleotides):
					...
					mapped_tail_nucleotides=match_read_nucleotides.group("genome_encoded_tail")
					...
					R5_mapping_pos = int(R5_mapping_pos)-number_mapped_tail_nucleotides #modify mapping position
				...
				# -1 means that read was unmapped
					if (m2.group('pos')!="-1"): #mapping of R3 read
						...
						if (clip3_R5_length==clip3_R3_length):
							...
							if (tailseq_tail_length==clip3_R3_length):
								...
								my_regex = r"^(?P<tail>" + re.escape(clipped_R3) + r")(?P<rest_of_seq>.*)"
								tail_match = re.match(my_regex,clipped_R5)
								#find R5 tail in R5 tail
								my_regex2 = r"^(?P<tail>" + re.escape(clipped_R5) + r")(?P<rest_of_seq>.*)"
						...
						if (tailseq_tail_length==clip3_R5_length):
							...
							tail_match = re.match(regex_for_heuristic_tail_identification_R3,str(clipped_R5))
							#changed R3 to R5 -in regex cause no CTGAC should occur in R5 clip
							if (tail_match):
								tails_results[seq_id]['processed_R5_clip']=tail_match.group('tail')
					...
					if (m2.group('pos')!="-1"): #mapping of R3 read
						if (tailseq_tail_length==clip3_R3_length):
							...
							#tailseq tail length have different length than clipping
							tail_match = re.match(regex_for_heuristic_tail_identification_R3,str(clipped_R3))
							if (tail_match):
				...
				if (m1.group('pos')!="-1"): #mapping of R5 read
					...
					if (m2.group('pos')!="-1"):
						...
						if (tail_match):
							...
							tails_results[seq_id]['tail_sequence']=tail_match.group('tail')
							tails_results[seq_id]['mapping_position']=R3_mapping_pos
						else:
		...
		AmixedU_match = re.match("^(?P<Atail>A+[TGCA]{0,5}?A+)(?P<Utail>T+[TGCA]{0,2}T+)$",tail_sequence)
		AmixedG_match = re.match("^(?P<Atail>A+[TGCA]{0,5}?A+)(?P<Utail>G+)$",tail_sequence)
		UmixedA_match = re.match("^(?P<Utail>T+[TGCA]{0,5}?T+)(?P<Atail>A+[TGCA]{0,2}A+)$",tail_sequence)
		UmixedG_match = re.match("^(?P<Utail>T+[TGCA]{0,5}?T+)(?P<Atail>G+)$",tail_sequence)
		AmixedUmixed_match = re.match("^(?P<Atail>A+[TGCA]{0,5}?A+)(?P<Utail>T+[TGCA]{0,2}T+$)",tail_sequence)
		...
		final_results[seq_id]['tail_length']=len(final_results[seq_id]['tail_sequence'])
...
#iterate through R5 files
	...
	if(len(R5_records)==0):
		...
		if (transcript!='ENDOL1'):
			...
			print("Skipping bowtie part because genomic LINE sequences are processed using RepeatMasker")
