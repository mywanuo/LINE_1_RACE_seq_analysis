...
tails_data_mapped_true_no_hetero_no_other <- tails_data_mapped_true_no_hetero_no_other[-grep("other",
    tails_data_mapped_true_no_hetero_no_other$tail_type), ]
...
levels(as.factor(tails_data_mapped_true_no_hetero_no_other_tails$tail_type))
...
tails_data_mapped_true_no_hetero_no_other_tails_NT1 <- tails_data_mapped_true_no_hetero_no_other_tails[(tails_data_mapped_true_no_hetero_no_other_tails$condition ==
    "NT" & tails_data_mapped_true_no_hetero_no_other_tails$localization == "TOTAL") |
    grepl("CYTO_NT", tails_data_mapped_true_no_hetero_no_other_tails$sample_name),
    ]
# filter out tails longer than 64nt - due to a limit of spike-ins used
...
pdf("GENOMIC_LINE_NT_tail_types_distribution_cell_line.pdf")
summary_tail_lengths_table_name <- paste("nt_summarized_tails_lengths_by_type")
assign(summary_tail_lengths_table_name, summarize_tails_by_tail_type(tails_data_mapped_true_no_hetero_no_other_tails_NT[tails_data_mapped_true_no_hetero_no_other_tails_NT$primer_name ==
    "L1NGS0", ]))
summary_tail_lengths_table <- eval(as.symbol(summary_tail_lengths_table_name))
plot_tail_lengths <- ggplot(summary_tail_lengths_table, aes(x = as.factor(cell_line),
    fill = tail_type, colours = tail_type)) + geom_bar(aes(y = freq), position = position_stack(),
    stat = "identity") + scale_y_continuous() + xlab("cell_line") + ylab("% of 3' ends") +
    scale_fill_grey() + ggtitle(paste("LINE1 reads tails distribution by type, L1NGS0, SSDNA spikes, mapped only"))
